...
class Team:
    def __init__(self, name, charter, alliance, prestige, color, status, sponsor, branding, commitment, chassis, performance, aero, gearbox, suspension, brakes, reliability, characteristics, wear, supplier, power, tires, grip, strategist):
        self.name = name
...
def weather_modifier(weather_condition, driver, team):
    ...
    if weather_condition == 'Rainy':
        ...
        if team.grip > 0:
            team.grip *= 0.875  # Reduce positive values
        else:
            team.grip /= 0.875  # Increase the magnitude of negative values

    elif weather_condition == 'Hot':
        driver.skill *= 0.975
        team.reliability *= 0.975
        if team.grip > 0:
            team.grip *= 1.125  # Increase positive values
        else:
            
            team.grip /= 1.125  # Reduce the magnitude of negative values
...
def simulate_qualifying(team, weather_condition, discipline, event, grid_size, circuit_type, series_name, script_directory, track_speed, track_characteristics):
    standings_data = read_standings_data(series_name, script_directory)
    ...
    for driver in drivers_copy:
        ...
        #         speed_modifier *= 1.5
        #         driver.speed *= speed_modifier
        #         driver.skill *= speed_modifier
        #     else:
        #         speed_modifier *= 1.75
        #         driver.speed *= speed_modifier
        #         driver.skill *= speed_modifier
        ...
        if discipline == 'Endurance':
            qualifying_result = (((team.average_speed + (team.average_skill / 2)) * special_modifier) + team.grip) + (team.performance * (1 - (0.75 * team.wear))) + team.power + randomness
            qualifying_results.append(qualifying_result)
        else:
            qualifying_result = (((driver.speed + (driver.skill / 2)) * special_modifier) + team.grip) + (team.performance * (1 - (0.75 * team.wear))) + team.power + randomness
            qualifying_results.append(qualifying_result)
...
def simulate_race(driver, team, starting_position, weather_condition, discipline, circuit_type, qualifying_results, sorted_qualifying_results, track_characteristics):
    ...
    if collided_drivers:
        ...
        for collided_drivers in collided_drivers:
            collided_drivers.dnf = "Collision"
            # Return a performance score of 0 for these drivers
            if collided_drivers == driver:
                return 0
    ...
    fantastic_chance + (driver.skill / 10000) + (team.reliability / 100)
    shocking_chance + ((100 - driver.skill) / 10000) + ((100 - team.reliability) / 100)
    ...
    if team.strategist == "Terrible":
        ...
        if strategy_value > 0.9:
            race_strategy = 10 - (team.grip * 1.1)
            ...
            race_strategy = -30 - (team.grip * 1.1)
            if any(trait in driver.traits for trait in ['Strategist']):
                race_strategy *= 0.85
            if any(trait in driver.traits for trait in ['GoodInstincts']):
                race_strategy *= 0.95
            ...
            race_strategy = -20 - (team.grip * 1.1)
            if any(trait in driver.traits for trait in ['Strategist']):
                race_strategy *= 0.85
            if any(trait in driver.traits for trait in ['GoodInstincts']):
                race_strategy *= 0.95
    ...
    if team.strategist == "Fair":
        ...
        if strategy_value > 0.75:
            race_strategy = 20 - (team.grip * 0.75)
            if any(trait in driver.traits for trait in ['Strategist']):
                race_strategy *= 1.15
            if any(trait in driver.traits for trait in ['GoodInstincts']):
                race_strategy *= 1.05
            ...
            race_strategy = -10 - (team.grip * 0.75)
            if any(trait in driver.traits for trait in ['Strategist']):
                race_strategy *= 0.85
            if any(trait in driver.traits for trait in ['GoodInstincts']):
                race_strategy *= 0.95
    ...
    if team.strategist == "Great":
        ...
        if strategy_value > 0.7:
            race_strategy = 25 - (team.grip * 0.5)
            if any(trait in driver.traits for trait in ['Strategist']):
                race_strategy *= 1.15
            if any(trait in driver.traits for trait in ['GoodInstincts']):
                race_strategy *= 1.05
            ...
            race_strategy = -15 - (team.grip * 0.5)
            if any(trait in driver.traits for trait in ['Strategist']):
                race_strategy *= 0.85
            if any(trait in driver.traits for trait in ['GoodInstincts']):
                race_strategy *= 0.95
    ...
    if team.strategist == "Excellent":
        ...
        strategy_value = random.uniform(0, 1)
        if strategy_value > 0.7:
            race_strategy = 30 - (team.grip * 0.4)
            if any(trait in driver.traits for trait in ['Strategist']):
                race_strategy *= 1.15
            if any(trait in driver.traits for trait in ['GoodInstincts']):
                race_strategy *= 1.05
            ...
            race_strategy = -10 - (team.grip * 0.4)
            if any(trait in driver.traits for trait in ['Strategist']):
                race_strategy *= 0.85
            if any(trait in driver.traits for trait in ['GoodInstincts']):
                race_strategy *= 0.95
    ...
    #     if random.uniform(0, 1) < 0.75:
    #         skill_modifier *= 1.5
    #         driver.speed *= skill_modifier
    #         driver.skill *= skill_modifier
    #     else:
    #         skill_modifier *= 2
    #         driver.speed *= skill_modifier
    #         driver.skill *= skill_modifier
    ...
    if discipline == 'Endurance':
        race_result = (((team.average_speed / 2) + team.average_skill) * special_modifier) + (team.performance * (1 - (0.75 * team.wear))) + team.power + race_strategy + randomness
        race_result -= penalty_factor # Better starting position gives a performance boost
    else:
        race_result = (((driver.speed / 2) + driver.skill) * special_modifier) + (team.performance * (1 - (0.75 * team.wear))) + team.power + race_strategy + randomness
        race_result -= penalty_factor # Better starting position gives a performance boost
...
def simulate_collision(sorted_qualifying_results):
    ...
    for i, ((team_name, driver_name), (driver, qualifying_result)) in enumerate(sorted_qualifying_results):
        ...
        # Randomly select one `j` from the candidates, if any
        if proximity_candidates:
            selected_j = random.choice(proximity_candidates)
            _, (other_driver, other_qualifying_result) = sorted_qualifying_results[selected_j]
            ...
            if random.uniform(0, 1) > min(driver.skill / 100, 0.999) and random.uniform(0, 1) > crash_probability:
                if random.uniform(0, 1) < 0.085:
                    if driver.dnf != "Collision" and other_driver.dnf != "Collision":  # Avoid marking the driver multiple times
...
def calculate_fastest_lap_and_laps_led(sorted_qualifying_results, dnq_results, sorted_race_results, total_laps, base_time):
    ...
    for i, (team_name, driver_names, race_result) in enumerate(sorted_race_results):
        ...
        for driver in driver_names.split(", "):  # Split in case there are multiple drivers
            ...
            laps_led[driver] += int(base_laps + (1 - position_factor) * (total_laps / 2) + race_result_factor - random.uniform(0, 30))
...
def read_standings_data(series_name, script_directory):
    ...
    for team in teams.values():
        for driver in team.drivers:
            ...
            if result is not None:  # Skip if qualifying result is None
                ...
                if discipline == 'Endurance':
                    # For endurance, use combined name and store results for all drivers together
                    combined_name = combine_driver_names(team.drivers)
                    qualifying_results[(team.name, combined_name)] = qualified_drivers  # Store qualifying results
    ...
    for i, ((team_name, driver_name), _) in enumerate(sorted_qualifying_results):
        ...
        if team.drivers:  # Check if the team has at least one driver
            if discipline == 'Endurance':
                ...
                race_results.append((team_name, combine_driver_names(team.drivers), race_result))
            else:
                race_result = simulate_race(team.drivers[0], team, starting_position, weather_condition, discipline, circuit_type, qualifying_results, sorted_qualifying_results, track_characteristics)
