...
class Driver:
    def __init__(self, name, nationality, age, speed, skill, experience, morale, preferred_discipline, preference, traits, fame, reputation, partner, contract, target, relations, team_name):
        ...
        self.entry_condition = 100

class Team:
    def __init__(self, name, charter, alliance, prestige, color, status, sponsor, branding, commitment, chassis, performance, aero, gearbox, suspension, brakes, reliability, characteristics, wear, supplier, engine, power, tires, strategist):
        self.name = name
...
def simulate_race(driver, team, starting_position, weather_condition, discipline, circuit_type, qualifying_results, sorted_qualifying_results, track_characteristics, iteration, iterations):
    ...
    simulate_retirement(driver, team, starting_position, qualifying_results, iteration, iterations)

    # Store the original skill value
    original_skill = driver.skill
    original_speed = driver.speed

    randomness = random.uniform(-40, 40)
    ...
    if weather_condition == 'Clear':
        ...
        race_strategy *= 1.5
        randomness = random.uniform(-70, 70)
    ...
    if circuit_type in ['Superspeedway']:
        penalty_factor = (starting_position - (starting_position * 0.75)) * (15 * (1 + (2 * iteration / iterations)))
        randomness *= 2.25
    elif circuit_type in ['Oval']:
        penalty_factor = (starting_position - (starting_position * 0.75)) * (17.5 * (1 + (2 * iteration / iterations)))
        randomness *= 1.75
    elif circuit_type in ['Short Track']:
        penalty_factor = (starting_position - (starting_position * 0.75)) * (20 * (1 + (2 * iteration / iterations)))
        randomness *= 1.25
    elif circuit_type in ['Road Course', 'Grand Prix']:
        penalty_factor = (starting_position - (starting_position * 0.25)) * (15 * (1 + (2 * iteration / iterations)))
    elif circuit_type in ['Street Track']:
        penalty_factor = (starting_position - (starting_position * 0.25)) * (20 * (1 + (2 * iteration / iterations)))
    else:
        penalty_factor = (starting_position - (starting_position * 0.25)) * (13 * (1 + (2 * iteration / iterations)))
    ...
    randomness *= max(0.1, 1 - (iteration / (2 * iterations)))
...
def simulate_retirement(driver, team, starting_position, qualifying_results, iteration, iterations):
    ...
    race_wear = 1 - (iteration / (0.25 * iterations))
    ...
    if random.uniform(0, 1) > reliability:
        if random.uniform(0, 1) > threshold:
            if random.uniform(0, 1) > race_wear:
                driver.entry_condition -= 25

    if iteration == 0 or iteration == 1:
        if any(status in team.status for status in ['StartAndPark']) and starting_position > top_30:
            if random.uniform(0, 1) > reliability:
                if random.uniform(0, 1) > threshold:
                    driver.entry_condition -= 100
    ...
    with open(csv_file_path, mode='r', newline='', encoding='utf-8') as csvfile:
        ...
        for row in reader:
            if row['Series'] == series_name:
                ...
                retirement_threshold = int(row['Retirements'])  # Retirement threshold for mechanical issues
                return discipline, region, tier, team_rules, chassis_rules, engine_rules, charter_system, charter_slots, retirement_threshold
...
def main():
    ...
    discipline, region, tier, team_rules, chassis_rules, engine_rules, charter_system, charter_slots, retirement_threshold = get_series_attributes_from_csv(series_name, script_directory)
    ...
    iterations = int((total_laps * base_time) / 1000)  # Initialize iterations before using it
    ...
    for iteration in range(iterations):
        ...
        for i, (team_name, driver_name, _) in enumerate(sorted_race_results):
            ...
            if team.drivers:  # Check if the team has at least one driver
                ...
                if driver.dnf == "Retirement" or driver.dnf == "Crash":
                    race_result = 0  # Mark the race result as 0 for retirement
                elif driver.entry_condition < retirement_threshold:
        ...
        if iteration < iterations - 1:
            ...
            for pos, (team_name, driver_name, race_result) in enumerate(sorted_race_results):
                ...
                if driver:
                    condition = driver.entry_condition
                    print(f"{pos + 1}. {driver_name} ({team_name}) with score {race_result} and condition {condition}")
            print("\n")
    ...
    with Live(build_leaderboard_text(revealed_results), refresh_per_second=4, console=console) as live:
        for team_driver, qualifying_result in randomized_results:
            ...
            # time.sleep(0.45)  # Delay to simulate qualifying results coming in progressively
            time.sleep(0)
   ...
   # Print DNQ drivers
    ...
    for i, (team_name, driver_name, race_result) in enumerate(sorted_race_results):
        ...
        if i <= top_20 and i <= driver.target:
        ...
        if i >= bottom_20 and i >= driver.target:
            if random.uniform(0, 1) < 0.1:  # Adjust the probability as needed
                if random.uniform(0, 1) < 0.5:
